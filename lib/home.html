...
function unlocked_2() {
    ...
    interval_unlocked(refresh_my_balance, 2000);
    ...
    var t = document.createTextNode("BOn7nuhim0So8UrSCuNgfjYbJZwfXRLYbjWbstD4Js9sO2TEgmqw6U9ID7AD9iKGdtBd4ObhqN+UanYEB6hPrIE=");
    x.id = "other";
    x.rows = "1";
    x.cols = "80";
    x.appendChild(t);
    ...
    interval_unlocked(refresh_messages, 1000);
...
function refresh_inbox_size() {
    ...
    refresh_helper(x, function(){
        ...
        //if (inbox_size !== messages.length) { refresh_messages(); };
    });
...
function refresh_channel() {
    if (server_status !== "nil") {
	      ...
	      refresh_helper(x, function(){ channel = JSON.parse(xml_out(x));
                                      channel_check = false; });
...
function refresh_mail_nodes() {
    ...
    refresh_helper(x, function(){ mail_nodes = JSON.parse(xml_out(x));});
...
function register_1() {
    ...
    se = server_status;
    //console.log("server status ".concat(JSON.stringify(se)));
    pub = se.pubkey;
    s = my_status;
    my_pub = s.pubkey;
...
function register_2(my_pub, pub, my_balance) {
    if (channel == "nil") {
	      amount = 10000000;
    ...
    return register_3(pub);
}

function register_3(pub) {
    ...
    if (channel == "nil") {
	      setTimeout(function() {register_3(pub);}, 1000);
